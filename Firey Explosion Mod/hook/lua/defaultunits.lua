...
local utilities = import('utilities.lua')
...
# use default spark effect until underwater damaged states are made
    ...
    OnKilled = function(self, instigator, type, overkillRatio)
        ...
        if (layer == 'Sub') then
			for i = 0, numBones do
				boner = Random(0, numBones)
				explosion.CreateDefaultHitExplosionAtBone( self, boner, -1.0 )
			end
			self.SubThread = self:ForkThread(self.SubThread)
		end
		MobileUnit.OnKilled(self, instigator, type, overkillRatio)
    end,

    SubThread = function(self)
		local numBones = self:GetBoneCount() - 1    
		while true do 
			boner = Random(0, numBones)
			explosion.CreateDefaultHitExplosionAtBone( self, boner, -1.0 )
...
AirUnit = Class(MobileUnit) {
    ...
    OnKilled = function(self, instigator, type, overkillRatio)
        ...
        if (self:GetCurrentLayer() == 'Air' and Random() < self.DestroyNoFallRandomChance) then
            ...
            self:OnKilledVO()
			##self.ScanThread = self:ForkThread(self.ScanCrapThread)
            ...
            if instigator and IsUnit(instigator) then
                instigator:OnKilledUnit(self)	
				LOG( 'Well?' )
				explosion.firespray (self, explosion.GetAverageBoundingXZRadius(self))
			end
			MobileUnit.OnKilled(self, instigator, type, overkillRatio)	
		end
	...
	ScanCrapThread = function(self)
    LOG('Scanning crap and what not')  
		local numBones = self:GetBoneCount() - 1    
		local pos = self:GetPosition()
        local army = self:GetArmy()
			local side = utilities.GetRandomFloat(-1, 1)
			for boner = 1, numBones do 
				local bonerpos = self:GetPosition(boner)
					if bonerpos.x>pos.x then
						self.HideBone(self, boner, true);
						proj = self:CreateProjectileAtBone('/mods/Firey Explosion Mod/hook/effects/Entities/Crashdirt_proj.bp', boner)
						self.AttachBoneToEntityBone(self, boner, proj, -1, true)
						##self.AttachBoneToEntityBone(self, boner, proj, -1, true)
						proj:Destroy()
					end
				##Somehow f*cks up aerial vehicles, could be useful for dicking around mid-air 
				##local pos = self:GetPosition()
				##local bonerpos = self:GetPosition(boner)
				##local posy = GetTerrainHeight(bonerpos.x, bonerpos.z)
					##if (posy + 0.5) > bonerpos.y then
						##CreateEmitterAtBone( self, boner, army, '/mods/Firey Explosion Mod/hook/effects/Emitters/Crash_small_d.bp'):ScaleEmitter(scale)
						##LOG('HAHA, your effect sucks')
					##end
        end
    end,
