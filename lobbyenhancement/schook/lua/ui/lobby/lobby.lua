		...
		for i = 1, LobbyComm.maxPlayerSlots do
			if not gameInfo.ClosedSlots[i] then
				if gameInfo.PlayerOptions[i] and gameInfo.PlayerOptions[i].LEM and tonumber(string.sub(gameInfo.PlayerOptions[i].LEM[1], -3)) > myLEMVersion and not outdatedWarning then
					...
					# Display Pop-up or something about old version
...
local function TransmitMod(playerID, ModId)
	...
	local done = false
	repeat
		if not DiskGetFileInfo('/mods/'..ModInfo.uid..ModInfo.version..'.lua') then return end
...
function SetSlotInfo(slot, playerInfo)
	...
	if playerInfo.LEM then
		if isLocallyOwned then
			...
			local myLEMData = EnhancedLobby.GetLEMData()
    ...
    if lobbyComm:IsHost() then
		...
		if handicapMod then
			GpgNetSend('PlayerOption', string.format("handicap %s %d %s", playerInfo.PlayerName, slot, playerInfo.Handicap))
		end
...
function ReturnToMenu()
    ...
    --exitfn()
	import('/lua/ui/menus/main.lua').MenuShow()
end
...
local function TryLaunch(skipNoObserversCheck, skipSandboxCheck)
	...
	elseif gameInfo.GameOptions['Victory'] == 'sandbox' and totalHumanPlayers > 1
	and not skipSandboxCheck then
		UIUtil.QuickDialog(GUI, "Victory condition is set to Sandbox. Do you still wish to launch?",
						   "<LOC _Yes>", function() TryLaunch(false, true) end,
						   "<LOC _No>", nil,
						   nil, nil, 
						   true,
						   {worldCover = false, enterButton = 1, escapeButton = 2}
...
local function UpdateGame()
	...
	if teamSetting != gameInfo.GameOptions['TeamSpawn'] and gameInfo.GameOptions['TeamSpawn'] != 'random' then
		...
		GUI.teamsCombo:AddItems({'2','3','4','T/B','L/R'})
		local tooltips = {'lob_teamsetup_2','lob_teamsetup_3','lob_teamsetup_4','lob_teamsetup_TB','lob_teamsetup_LR'}
	...
	elseif teamSetting != gameInfo.GameOptions['TeamSpawn'] and gameInfo.GameOptions['TeamSpawn'] == 'random' then
		teamSetting = gameInfo.GameOptions['TeamSpawn']
		GUI.teamsCombo:ClearItems()
		GUI.teamsCombo:AddItems({'2','3','4'})
		local tooltips = {'lob_teamsetup_2','lob_teamsetup_3','lob_teamsetup_4'}
		Tooltip.AddComboTooltip(GUI.teamsCombo, tooltips)
	end
...
function CreateUI(maxPlayers, useSteam)
    ...
    --     GUI.panel = Bitmap(GUI, UIUtil.SkinnableFile("/scx_menu/lan-game-lobby/[ser]lobby.png"))
    ...
    GUI.panelWideLeft = Bitmap(GUI, '/textures/ui/common/scx_menu/lan-game-lobby/wide/[ser]wide.png')
		LayoutHelpers.CenteredLeftOf(GUI.panelWideLeft, GUI.panel, -11)
		GUI.panelWideLeft.Left:Set(function() return GUI.Left() end)
	GUI.panelWideRight = Bitmap(GUI, '/textures/ui/common/scx_menu/lan-game-lobby/wide/[ser]wide.png')
...
-- !!!
    ...
    -- LayoutHelpers.AtLeftTopIn(GUI.observerPanel, GUI.panel, 40, 660)
    -- GUI.observerPanel.Width:Set(706)
    -- GUI.observerPanel.Height:Set(114)
    GUI.observerPanel.Width:Set( 278 )
    if lobbyComm:IsHost() then
        GUI.observerPanel.Height:Set( 84 )
        LayoutHelpers.AtLeftTopIn(GUI.observerPanel, GUI.panel, 512, 620)
    else
